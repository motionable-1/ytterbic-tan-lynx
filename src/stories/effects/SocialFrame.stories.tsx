...
const meta: Meta<typeof SocialFrame> = {
  ...
  argTypes: {
    platform: {
      control: "select",
      options: ["instagram", "tiktok", "youtube", "twitter", "linkedin"],
    },
...
export const AllPlatforms: Story = {
  ...
  render: () => (
    <AbsoluteFill
      style={{ display: "flex", flexWrap: "wrap", gap: 16, padding: 20 }}
    >
      {(["instagram", "tiktok", "youtube", "twitter", "linkedin"] as const).map(
        (p, i) => (
          <div
            key={p}
            style={{ width: "48%", height: 200, position: "relative" }}
          >
            <SocialFrame
              platform={p}
              username={`@user_${p}`}
