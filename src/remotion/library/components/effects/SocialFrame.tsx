...
const row = (gap: number): React.CSSProperties => ({
  display: "flex",
  alignItems: "center",
  gap,
});
...
const Heart: React.FC<IP> = ({ size: s = 24, color: c = "#fff" }) =>
  sv(
    ...
    "none",
    <path
      d="M12 21s-7-4.35-7-10A4.5 4.5 0 0 1 12 6a4.5 4.5 0 0 1 7 5c0 5.65-7 10-7 10z"
      stroke={c}
      strokeWidth={2}
...
const Comment: React.FC<IP> = ({ size: s = 24, color: c = "#fff" }) =>
  sv(
    ...
    "none",
    <path
      d="M21 12a9 9 0 0 1-9 9l-4 1 1-4a9 9 0 1 1 12-6z"
      stroke={c}
      strokeWidth={2}
      strokeLinejoin="round"
    />,
...
const Music: React.FC<IP> = ({ size: s = 24, color: c = "#fff" }) =>
  sv(
    ...
    "none",
    <>
      <path d="M9 18V5l12-2v13" stroke={c} strokeWidth={2} />
...
const Avatar: React.FC<{ size: number; color: string; rounded?: boolean }> = ({
  size,
  color,
  rounded = true,
}) => (
...
function Instagram(
  { u, v, eng, on, dur, del, lk, cm, sh }: RP,
  ac: string,
  fr: number,
  fps: number,
) {
  const a0 = useAnim(fr, fps, on, dur, del, 0),
    a1 = useAnim(fr, fps, on, dur, del, 1),
    a2 = useAnim(fr, fps, on, dur, del, 2);
  return (
    <>
      <div
        style={{
          ...abs({ top: 0, left: 0, right: 0, bottom: 0 }),
          borderRadius: 16,
          padding: 3,
          background:
    ...
    <>
      {eng && (
        ...
        >
          ...
          <div style={col}>
            <Heart size={32} />
            <span style={ct}>{lk}</span>
          </div>
          <div style={col}>
            <Comment size={30} />
            <span style={ct}>{cm}</span>
          </div>
          <div style={col}>
            <Share size={28} />
            <span style={ct}>{sh}</span>
          </div>
          <div style={col}>
            <Music size={28} />
          </div>
...
function Youtube(
  ...
  const cur = Math.round(fr / fps),
    total = Math.round(durationInFrames / fps);
  return (
    <>
      <div
        style={{
...
function Twitter(
  { u, h, v, eng, on, dur, del, lk, cm, sh }: RP,
  ac: string,
  fr: number,
  fps: number,
) {
...
function Linkedin(
  { u, h, eng, on, dur, del, lk, cm, sh }: RP,
  ac: string,
  fr: number,
  fps: number,
) {
  ...
  return (
    <>
      ...
      {eng && (
        ...
        >
          <div style={btn}>
            <Heart size={18} color={m} />
            <span>Like {lk}</span>
          </div>
          <div style={btn}>
            <Comment size={18} color={m} />
            <span>Comment {cm}</span>
          </div>
          <div style={btn}>
            <Retweet size={18} color={m} />
            <span>Repost {sh}</span>
          </div>
          <div style={btn}>
            <Send size={18} color={m} />
            <span>Send</span>
          </div>
        </div>
      )}
...
export const SocialFrame: React.FC<SocialFrameProps> = ({
  ...
  const overlay = () => {
    switch (platform) {
      case "instagram":
        return Instagram(p, ac, frame, fps);
      case "tiktok":
        return Tiktok(p, ac, frame, fps);
      case "youtube":
        return Youtube(p, ac, frame, fps);
      case "twitter":
        return Twitter(p, ac, frame, fps);
      case "linkedin":
        return Linkedin(p, ac, frame, fps);
    }
  };
